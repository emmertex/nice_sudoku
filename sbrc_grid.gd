    ...
    for row in range(9):
        for col in range(9):
            ...
            if value != 0:
                ...
                # Add to row sector
                sector_data[row][digit].set(col)
                row_data[row][digit].set(col)
                
                # Add to column sector
                sector_data[col + 9][digit].set(row)
                col_data[col][digit].set(row)
                
                # Add to box sector
                sector_data[box + 18][digit].set(Cardinals.BxyN[row * 9 + col])
                box_data[box][digit].set(Cardinals.BxyN[row * 9 + col])
...
func get_candidates_for_cell(row: int, col: int) -> BitSet:
    ...
    for digit in range(9):
        if row_data[row][digit].get(col):
            result.clear(digit)
    
    # Remove candidates from column
    for digit in range(9):
        if col_data[col][digit].get(row):
            result.clear(digit)
    
    # Remove candidates from box
    var box = Cardinals.Bxy[row * 9 + col]
    var box_pos = Cardinals.BxyN[row * 9 + col]
    for digit in range(9):
        if box_data[box][digit].get(box_pos):
            result.clear(digit)
...
func set_cell_value(row: int, col: int, value: int):
    ...
    if old_value != 0:
        ...
        # Remove from sectors
        sector_data[row][old_digit].clear(col)
        row_data[row][old_digit].clear(col)
        sector_data[col + 9][old_digit].clear(row)
        col_data[col][old_digit].clear(row)
        sector_data[box + 18][old_digit].clear(Cardinals.BxyN[row * 9 + col])
        box_data[box][old_digit].clear(Cardinals.BxyN[row * 9 + col])
