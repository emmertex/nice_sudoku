...
const PENCIL_NUM = [1,2,3,4,5,6,7,8,9]
var puzzles: Dictionary = {"easy": "res://puzzles/easy.txt", "medium": "res://puzzles/medium.txt", 
	"hard": "res://puzzles/hard.txt", "expert": "res://puzzles/diabolical.txt"}
...
func _update_RnCnBn():
	...
	# 	for col in range(9):
	# 		for test in range(9):
				#Rn[row][col] &= ~(1 << (grid[row][test] - 1))
				#Cn[row][col] &= ~(1 << (grid[test][col] - 1))
				#Bn[row][col] &= ~(1 << (grid[(row/3)*3 + test/3][(col/3)*3 + test%3] - 1))
...
func is_valid_move(row: int, col: int, num: int) -> bool:
	...
	#var box_mask:int = Bn[(row/3)*3 + col/3][row%3*3 + col%3] & mask
	#print("row_mask: ", row_mask, ", col_mask: ", col_mask, ", box_mask: ", box_mask)
...
func auto_fill_pencil_marks():
	for row in range(9):
		for col in range(9):
			var valid_numbers = get_valid_numbers(row, col)
...
# 							store_pencil(row, col, num, true, false)


func get_valid_numbers(row: int, col: int) -> Array:
	var valid_numbers = []
	if grid[row][col] != 0:
...
func is_original_number(row: int, col: int) -> bool:
